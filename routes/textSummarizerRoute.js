...
const { textSummaryPrompt } = require('../utils/prompts');

const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });

const generateSummary = async (summarizationPrompt) => {
    try {
...
router.post('/', async (req, res) => {
    try {
        const { lengthSummary, inputText } = req.body;

        console.log(lengthSummary, inputText);
        const summarizationPrompt = await textSummaryPrompt(lengthSummary, inputText);
